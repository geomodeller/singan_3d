...
# except ImportError:
#     print("nibabel not found. Install if needed for NIfTI files.")
# Optional import for visualization
try:
    import matplotlib.pyplot as plt
except ImportError:
    print("matplotlib not found. Install if needed for visualization.")
...
def load_real_volume(path, device):
    ...
    if vol.ndim == 3: # Add channel dimension if missing
        ...
        if vol.shape[-1] == 1: # Assume (D, H, W, C), permute
             ...
             raise ValueError("Input volume has 4 dimensions but first is not 1 (channel). Please provide single channel data.")
        # If shape == 1, assume it's already (C, D, H, W)
    else:
...
def save_volume(vol_tensor, path):
    ...
    if path.endswith('.npy'):
        ...
        print(f"Warning: Unsupported save format for {path}. Saving as.npy")
        np.save(path.replace(os.path.splitext(path), '.npy'), vol_np)
...
def train_3d_singan(real_volume, opt, device):
    ...
    print(f"Created pyramid with {num_scales} scales.")
    for i, vol in enumerate(pyramid):
        print(f"  Scale {num_scales - 1 - i} (Coarsest={i==0}): {vol.shape}")
    ...
    for n in range(num_scales): # n iterates through pyramid list [0, num_scales-1]
        ...
        opt.nc_im = 1 # real_vol_at_scale.shape # Should be 1
        current_size = real_vol_at_scale.shape[-3:]
        print(f"Volume size: {current_size}")
        ...
        for iter_num in range(opt.niter):
            ...
            # Log progress
            if iter_num % 100 == 0:
                print(f"Scale [{scale_idx_actual}] Iter [{iter_num}/{opt.niter}] Loss_D: {errD.item():.4f} Loss_G: {errG.item():.4f} (Adv: {errG_adv.item():.4f} Rec: {errG_rec.item():.4f})")
...
if __name__ == "__main__":
    ...
    if opt.mode == 'train':
        ...
        print(f"Loaded real volume: {real_volume.shape}")
        opt.nc_im = real_volume.shape # Set number of input channels
        ...
        print(f"Trained model saved to {os.path.join(opt.outdir, 'trained_model_3d.pth')}")
        ...
        dummy_pyramid = [torch.empty(tuple(s)) for s in pyramid_shapes] # Coarsest to finest
        ...
        if opt.gen_size:
            try:
                ...
                print(f"Generating with custom size: {custom_shape}")
            except:
                print(f"Error: Invalid gen_size format '{opt.gen_size}'. Use D,H,W (e.g., '32,128,128'). Exiting.")
                exit()
        ...
        print(f"Error: Unknown mode '{opt.mode}'")